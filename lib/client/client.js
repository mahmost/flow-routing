...
AccountsTemplates.getparamToken = function() {
  ...
  return router.currentRoute.params['paramToken'];
};
...
if (window.app && window.app.$router) window.app.$router.afterEach(function(from, to) {
...
AccountsTemplates.linkClick = function(route) {
  ...
  var router = window.app.$router;
  if (path === '#' || AccountsTemplates.avoidRedirect || path === router.currentRoute.path) {
...
AccountsTemplates.logout = function() {
  ...
  Meteor.logout(function() {
    if (onLogoutHook) {
      ...
      router.push(homeRoutePath); // redirect
    }
  });
};
...
AccountsTemplates.postSubmitRedirect = function(route) {
  ...
  if (AccountsTemplates.avoidRedirect) {
    ...
    if (AccountsTemplates.redirectToPrevPath) {
      router.replace(AccountsTemplates.getPrevPath()); // redirect
    }
  } else {
    ...
    if (nextPath) {
      ...
      if (previousPath && router.currentRoute.path !== previousPath) {
        router.replace(previousPath);
      } else {
        var homeRoutePath = AccountsTemplates.options.homeRoutePath;
        if (homeRoutePath && router) {
...
/* Seems not needed for VueRouter
// Initialization
